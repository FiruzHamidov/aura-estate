...
`GET /up` proves Laravel/PHP availability but does not prove Reverb health. Release readiness also requires a WebSocket connection smoke test through the public proxy, successful/denied auth checks for the three private channel classes, and observation that a committed test message is received once while a rolled-back write is never received. The current production deploy script checks only the Laravel health route and queue worker, so production realtime runtime is **not proven ready by repository automation alone**. Add the Reverb Supervisor status and WebSocket smoke check before enabling the feature flag.
