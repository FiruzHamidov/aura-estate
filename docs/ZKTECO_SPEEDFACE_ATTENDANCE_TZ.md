...
Результат: `37 passed`, `217 assertions`.
...
Полный набор репозитория на момент проверки: `411 passed`, `44 failed`. Все 37 тестов модуля посещаемости проходят; оставшиеся падения воспроизводятся вне модуля и относятся к существующим тестовым схемам CRM/KPI и другим старым ожиданиям (например, отсутствующие в тестовой SQLite-схеме таблицы/столбцы).
...
Повторный deployment, поддержка всех активных ролей и исправленный UTC-cast подтверждены на production. Терминал имеет состояние `online`, расхождение часов составляет 1 секунду, `last_error=null`, активных сопоставлений — 1. Во время production-проверки обнаружено, что `work_date` сериализуется как UTC datetime предыдущего календарного дня; локально исправлено на строку `YYYY-MM-DD` и покрыто API-тестом. Открытые gate пилота: deployment исправления `work_date` и физическая проверка офлайн-доставки.
