...
Фактический ATTLOG терминала `WCF3254200047` получен 2026-08-16 и сохранён как обезличенный fixture `tests/Fixtures/zkteco/zam230_wcf3254200047_attlog.txt`. Пакет подтвердил совместимость парсера с прошивкой ZAM230: передаются идентификатор пользователя, локальное время, статус, способ проверки, work code и служебные поля. Биометрические шаблоны, фото и данные RFID в ATTLOG отсутствуют.
...
Результат: `35 passed`, `182 assertions`.

Автоматически проверены:

- root routing и `text/plain` ответы TA PUSH;
- сквозной проход реального ATTLOG ZAM230 от Device API до дневной сводки;
...
Реальный проход лица принят от устройства: Device User ID `1`, локальное время `2026-08-16 19:50:49`, `verify_mode=15`. Событие намеренно осталось в состоянии `unmapped`, поскольку Aura User ID 1 — администратор и не относится к ролям, для которых ведётся посещаемость. Для завершения пилота необходимо назначить на терминале идентификатор активного агента, создать явное сопоставление и повторить проход. Также требуется проверить доставку накопленного события после временного отключения сети.

Повторная read-only проверка production показала, что версия с исправленным UTC-cast ещё не развёрнута: API возвращает для этого события `last_event_at=2026-08-16T09:50:49Z`, тогда как корректное значение — `2026-08-16T14:50:49Z`. В текущей production-записи устройства `user_mappings_count=0`. Эти два пункта являются открытыми gate пилота: повторный deployment текущего backend и сопоставление Device User ID активного агента.
