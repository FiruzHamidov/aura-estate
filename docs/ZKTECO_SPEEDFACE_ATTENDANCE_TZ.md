...
Физический офлайн-тест выполнен 2026-08-16: сеть терминала отключена, в `20:34:57` по Душанбе зарегистрирована третья отметка, сеть восстановлена в `20:36`. ATTLOG фактически содержал статус `2` и `verify_mode=15`. После восстановления устройство передало исходное время события, а `last_event_at` на backend обновился до `2026-08-16T15:34:57Z`. Это подтверждает локальное накопление и последующую доставку ATTLOG.

После deployment поддержки перерывов и запуска queue worker raw event `3` нормализован как `break_out`, `direction=out`, `verification_method=face`, `is_duplicate=false`; время создания нормализованной записи — `20:44:49` по Душанбе. Контрольный повторный запрос сохранил итог ровно в три события и ноль семантических дублей. Дневная сводка корректна: `work_date=2026-08-16`, приход `19:50:49`, уход `20:03:09`, `events_count=3`, `worked_minutes=12`, `status=present`; отметка перерыва после ухода не подменила уход. Несопоставленных событий нет, терминал находится в состоянии `online`, `last_error=null`. Production-пилот backend завершён успешно.
