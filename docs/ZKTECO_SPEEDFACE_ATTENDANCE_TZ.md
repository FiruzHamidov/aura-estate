...
Результат: `37 passed`, `215 assertions`.
...
Два реальных прохода лица приняты от устройства: Device User ID `1`, локальное время `19:50:49` со статусом `0` и `20:03:09` со статусом `1`, `verify_mode=15`. Владелец Device User ID `1` подтверждён: это активный Aura User ID `1` с ролью администратора. После изменения ТЗ сопоставление `1 → 1` разрешено и при его создании оба события должны автоматически сформировать дневную сводку администратора. Также требуется проверить доставку накопленного события после временного отключения сети.

Повторный deployment и исправленный UTC-cast подтверждены на production: после второго прохода API возвращает корректное `last_event_at=2026-08-16T15:03:09Z`, терминал имеет состояние `online`, расхождение часов составляет 1 секунду, `last_error=null`. Изменение для всех активных ролей пока ожидает deployment. После него открытые gate пилота: создать сопоставление `1 → 1`, проверить сводку администратора и выполнить физическую офлайн-доставку.
