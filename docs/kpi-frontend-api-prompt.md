...
## 11) Sales Attribution (2026-05-29 update)
- Метрики `sold`, `rented`, `closed`, `sales_count`, `deals_count` могут быть дробными (`float`).
- Для совместных сделок используется равное деление `1/N` на backend.
- На фронте:
  - не приводить sales-метрики к int;
  - отображать до 2 знаков после запятой с trim trailing zeros;
  - сортировать по numeric float значениям;
  - не терять дробную часть в CSV/Excel экспортах.
- Подсказки в UI:
